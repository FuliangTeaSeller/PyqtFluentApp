...
from GUI.UI.tox21_result import Ui_Form
class ResultTox21(QFrame, Ui_Form):
    def __init__(self, molecule, parent=None):
        ...
        self.result()
    def result(self):
        self.scrollArea.setWidgetResizable(True) # 允许滚动区域自适应大小
        self.layout3 = QVBoxLayout(self.frame_2)
        font = QFont()
        ...
        font1.setFamily('宋体')
        # 输入模型,返回预测值
        output, photo = tox_21_main(self.molecule, 'single')
        #分子smiles
        self.label.setText('{}分子的二维结构图：'.format(self.molecule))
        ...
        self.label_14.setAlignment(Qt.AlignHCenter)
        '''font = QFont()
        font.setPointSize(15)
        font.setBold(False)
        font.setFamily('宋体')
        ...
        # 设置布局
        #self.scrollArea.setLayout(self.layout3)
        # 嵌套
